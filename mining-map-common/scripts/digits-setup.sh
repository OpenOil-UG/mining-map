...
mkdir -p /mining-map/digits
cd /mining-map/digits
if [ ! -d /mining-map/digits/.git ]; then
   echo "Cloning digits repo..."
   git clone https://github.com/NVIDIA/digits /mining-map/digits
   ...
   sed -i 's/DataRequired/Required/' /mining-map/digits/digits/dataset/forms.py
   sed -i 's/DataRequired/Required/' /mining-map/digits/digits/dataset/images/forms.py
   sed -i 's/DataRequired/Required/' /mining-map/digits/digits/model/forms.py
...
if [ ! -f /mining-map/digits/digits/digits.cfg ]; then
    echo "Preparing digits config..."
    cat <<EOF | tee /mining-map/digits/digits/digits.cfg
[DIGITS]
jobs_dir = /mining-map/digits/digits/jobs
log_file = /mining-map/digits/digits/digits.log
...
DAEMON_NAME="digits"
DAEMON_USER=""
DAEMON_PATH="/mining-map/digits/digits-devserver"
DAEMON_OPTS=""
DAEMON_PWD="/mining-map/digits/"
...
EOF
    ...
    # Using 99 because we want /mining-map to be mounted first
